...
export default function Sidebar({
  ...
  return (
    <aside aria-label="Application sidebar" className="flex flex-col w-56 shrink-0 bg-surface-800 border-r border-surface-600 h-screen select-none">
      {/* Logo */}
      <div className="flex items-center gap-2.5 px-5 py-5 border-b border-surface-600">
        <Shield size={22} className="text-brand-400" aria-hidden="true" />
          ...
          return (
            ...
            >
              ...
              {id === "staging" && pendingCount > 0 && (
                <span
                  aria-label={`${pendingCount > 99 ? "99+" : pendingCount} pending rules`}
                  className="ml-auto bg-brand-600 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center"
                >
